...
| Резать строку | **`MID$(A$,2,3)` есть, `LEFT$` и `SEG$` — Ошибка 13** | забой при вводе имени в таблицу рекордов |
| Строковые массивы | `DIM A$(4)`, присваивание и `+` — есть | имена в таблице рекордов |
| Число из строки | `VAL("123")` и `STR$(42)` — есть | в игре не понадобились |
...
Где живёт шрифт, выяснилось перехватом чтений памяти: **начертания лежат в ПЗУ
периферийного процессора** (латинская `A` — по адресу 0120743, по 11 байт на знак), а
**таблица указателей на них — в ОЗУ того же ПП**, по адресу 0014142 плюс код знака,
умноженный на два. Таблица пишется: подменив в ней указатель, мы заставили машину печатать
`A` там, где программа выводит `O`.

🚨 **Из Бейсика туда не дотянуться.** Это память ПП, а `POKE` центрального процессора её не
видит: в его адресном пространстве такой таблицы нет (поиск по всем четырём планам ОЗУ
шестнадцатью способами — по строкам и по столбцам, прямой и обратный порядок бит и строк,
с инверсией — не дал совпадений). Поэтому свой знак в этой программе не рисуется на старте,
а готовится до неё — **редактором шрифта с системной дискеты** (`FNT.SAV`, Alphaprog Font
Editor; см. [SCHOOL_UKNC.md](SCHOOL_UKNC.md)). Ровно так это и делалось в школе.
...
🚨 **Знак вдвое выше своей ширины.** 640×288 точек растянуты на экран 4:3, поэтому точка не
квадратная, и знакоместо 80×24 выглядит как прямоугольник 1:2. Из этого и исходит рисунок
игры: клетка стакана из **двух** узких знаков выходит квадратной. Веб-страница повторяет это
растяжением по вертикали — без него клетки выходили приплюснутыми, не как на машине.
